...
|---|---|---|---|---|---|
| CORE-001 | Reproducible Windows development setup | `PARTIAL` | M0 | Fresh clone installs, builds and launches using documented commands | SDK/MSVC rebuild passes; current `main` remains Bash-dependent, while unmerged PR #21 demonstrates `npm ci` and verification without Bash on Windows plus Linux validation; [#18](https://github.com/joputajones/tpv-abierto/issues/18) |
