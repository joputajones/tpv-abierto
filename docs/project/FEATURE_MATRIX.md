...
|---|---|---|---|---|---|
| CORE-001 | Reproducible Windows development setup | `DONE` | M0 | Fresh clone installs, builds and launches using documented commands | Integrated `main` at `a05cc79` passes clean install, native rebuild, Electron verification, builds and all 69 test scripts in PowerShell without Bash. Packaged release and physical operation are separate gaps; automated off-device recovery is covered by BACKUP-001/002 |
...
| CORE-004 | Local-only operation | `DONE` (`SIM/CI` only) | M1 | Core order flow works with internet disconnected | PR #41 and corrective PR #42 are merged; Windows/Ubuntu offline jobs, Linux baseline and post-merge `main` pass O-01…O-16 plus O-FP with loopback-only API/KDS/frontend/renderer, restarts and zero successful Internet connections. This does not cover physical disconnection, hostile LAN, phones, printers, a full shift, restaurant operation or fiscal compliance |
...
| NET-001 | Internet-loss continuity | `PARTIAL` | M1 | Local service continues while internet-only features pause | O-01…O-16 and O-FP pass on integrated `main` and dedicated Windows/Ubuntu CI with process-scoped external blocking and an approved loopback reconnection simulator. Physical disconnection, long-duration and multi-device LAN evidence remain pending |
