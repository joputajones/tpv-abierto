...
|---|---|---|---|---|---|
| CORE-001 | Reproducible Windows development setup | `DONE` | M0 | Fresh clone installs, builds and launches using documented commands | Integrated `main` at `28a3f8d` passes clean install, native rebuild, Electron verification, builds and all 71 test scripts in PowerShell without Bash. Packaged release and physical operation are separate gaps; automated off-device recovery is covered by BACKUP-001/002 |
...
| CORE-004 | Local-only operation | `DONE` (`SIM/CI` only) | M1 | Core order flow works with internet disconnected | PR #41/#42/#44 are merged; post-merge `main` and Full Cross-Platform Matrix `30711604415` pass O-01…O-16 plus O-FP, builds and unpacked packaging on Ubuntu, Windows and macOS x64/arm64 with loopback-only API/KDS/frontend/renderer, restarts and zero successful Internet connections. This does not cover physical disconnection, hostile LAN, phones, printers, a full shift, restaurant operation or fiscal compliance |
...
| BACKUP-001 | Local and portable backup creation | `DONE` | M1 | Production backup is integrity-checked, checksummed and exported off-device | PR #35/#33: Windows production API creates the exact four-file synthetic package; GitHub artifact is the only cross-job channel and Windows/Linux observe the same pre-open SHA-256. Evidence level `CI_CROSS_RUNNER` |
| BACKUP-002 | Restore procedure | `PARTIAL` | M1 | Clean install restores to expected state and a second operator can repeat it | PR #46 integrates the graphical assistant at `CODE/SIM/BUILD/CI`: disposable restore, reopen, synthetic write, sanitized report, A-01…A-16 and packaged launch pass on Windows, Ubuntu and macOS x64/arm64. A real non-technical human run remains pending; #39 stays open |
...
| NET-001 | Internet-loss continuity | `PARTIAL` | M1 | Local service continues while internet-only features pause | O-01…O-16 and O-FP pass on integrated `main`, dedicated Windows/Ubuntu CI and the four-target platform matrix with process-scoped external blocking and an approved loopback reconnection simulator. Physical disconnection, long-duration and multi-device LAN evidence remain pending |
