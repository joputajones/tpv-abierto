...
| BACKUP-002 | Restore procedure | `PARTIAL` | M1 | Clean install restores to expected state and a second operator can repeat it | Automated clean restore, reopen, sequence continuity, write-after-restore and B-01…B-07 are `DONE` at `CI_CROSS_RUNNER`; the documented physical blind second-person drill remains open as #34 |
