...
|---|---|---|---|---|---|
| CORE-001 | Reproducible Windows development setup | `DONE` | M0 | Fresh clone installs, builds and launches using documented commands | Integrated `main` passes clean install, native rebuild, Electron verification, builds and all 68 test scripts in PowerShell without Bash. Packaged release and physical operation are separate gaps; automated off-device recovery is covered by BACKUP-001/002 |
...
| CORE-004 | Local-only operation | `PARTIAL` | M1 | Core order flow works with internet disconnected | #40 branch evidence passes O-01…O-16 locally on Windows with loopback-only API/KDS/frontend/renderer, restarts and zero successful Internet connections. Remains `PARTIAL` until Windows/Linux CI and merge; this is `SIM/CI`, not restaurant or hardware evidence |
...
| NET-001 | Internet-loss continuity | `PARTIAL` | M1 | Local service continues while internet-only features pause | O-01…O-16 pass locally with process-scoped external blocking and an approved loopback reconnection simulator; dedicated Windows/Linux CI, physical disconnection and multi-device LAN evidence remain pending |
