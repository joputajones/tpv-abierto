...
|---|---|---|---|---|---|
| CORE-001 | Reproducible Windows development setup | `DONE` | M0 | Fresh clone installs, builds and launches using documented commands | PR #21 merged at `a51fa54`; post-merge `main` passes clean install, native rebuild, Electron verification, builds and all 66 test scripts in PowerShell without Bash. This closes #18 for development portability only; packaged release, off-device recovery and physical operation are separate gaps |
| CORE-002 | SQLite data persistence | `PARTIAL` | M0 | Confirmed order survives clean restart and abrupt app restart test | #30 branch passes R-01…R-12 with committed rows preserved, open transactions reverted, WAL recovered, five alternating cycles and reusable API/KDS ports; evidence is `SIM` and remains pending review, Linux CI and merge |
