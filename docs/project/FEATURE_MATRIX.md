...
| BACKUP-002 | Restore procedure | `PARTIAL` | M1 | Clean install restores to expected state and a second operator can repeat it | Automated B-01…B-07 are `DONE` at `CI_CROSS_RUNNER`; #34 closed after a functional restore by a second operator on another computer, but advanced technical checks were incomplete. A non-technical assistant remains open as #39 |
