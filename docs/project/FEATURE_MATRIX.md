...
|---|---|---|---|---|---|
| CORE-001 | Reproducible Windows development setup | `DONE` | M0 | Fresh clone installs, builds and launches using documented commands | Integrated `main` at `a6724c56` passes clean install, native rebuild, Electron verification, builds and all 69 test scripts in PowerShell without Bash. Packaged release and physical operation are separate gaps; automated off-device recovery is covered by BACKUP-001/002 |
...
| BACKUP-002 | Restore procedure | `PARTIAL` | M1 | Clean install restores to expected state and a second operator can repeat it | A graphical assistant is implemented on the #39 branch with disposable restore, reopen, synthetic write, sanitized report and A-01…A-16 local evidence. Merge, cross-platform CI and a real non-technical human run remain pending; #39 stays open |
