...
|---|---|---|---|---|---|
| CORE-001 | Reproducible Windows development setup | `DONE` | M0 | Fresh clone installs, builds and launches using documented commands | PR #21 merged at `a51fa54`; post-merge `main` passes clean install, native rebuild, Electron verification, builds and all 65 test scripts in PowerShell without Bash. This closes #18 for development portability only; packaged release, off-device recovery and physical operation are separate gaps |
...
| CORE-003 | Safe database migration path | `PARTIAL` | M0 | Existing-data fixture upgrades with backup and no destructive loss | PR branch validates a verified fail-closed backup before v0→v38, source invariants and isolated retry; #16 remains open pending merge and released migrations remain unchanged |
