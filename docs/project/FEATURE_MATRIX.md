...
|---|---|---|---|---|---|
| CORE-001 | Reproducible Windows development setup | `BLOCKED` | M0 | Fresh clone installs, builds and launches using documented commands | SDK/MSVC rebuild now passes; literal `npm.cmd ci` still fails because Bash-dependent `postinstall` cannot find Git Bash; [#18](https://github.com/joputajones/tpv-abierto/issues/18) |
