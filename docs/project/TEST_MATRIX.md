...
The following results are integrated through PR #41 and corrective PRs #42/#44.
Dedicated Windows/Ubuntu jobs, Linux baseline, post-merge Windows `main` and
Full Cross-Platform Matrix `30711604415` on Ubuntu, Windows and macOS x64/arm64
...
| T-NET-005 | Internet-only service queues or fails clearly | `SIM/CI` | `DONE` | O-08…O-15 pass on integrated `main`, Windows/Ubuntu CI and the four-target full matrix with durable cloud outbox, zero Internet successes and fast guarded failures; real cloud interoperability and physical network failure remain separate gaps |
